...
int test_http_parser()
{
  ...
  { // test no overflow parse url
    std::string url("w://example.com:8001/signin/?123=1234");
    http_parser_url u;
    http_parser_url_init(&u);
    int rv = http_parser_parse_url(url.data(), url.size(), 0, &u);
    std::cout << url.substr(u.field_data[UF_SCHEMA].off, u.field_data[UF_SCHEMA].len) << std::endl;
    std::cout << url.substr(u.field_data[UF_HOST].off, u.field_data[UF_HOST].len) << std::endl;
    std::cout << url.substr(u.field_data[UF_PORT].off, u.field_data[UF_PORT].len) << std::endl;
    std::cout << url.substr(u.field_data[UF_PATH].off, u.field_data[UF_PATH].len) << std::endl;
    std::cout << url.substr(u.field_data[UF_QUERY].off, u.field_data[UF_QUERY].len) << std::endl;
    if (rv != 0) {
  ...
  for (int i = 0; i < 5; ++i) {
    for (int j = 0; j < messages[i].num_headers; ++j) {
      std::cout << messages[i].headers[j] << std::endl;
    }
  }
